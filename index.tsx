  ...
  static override styles = css`
    ...
    :host(:hover) svg {
      ...
      filter: drop-shadow(0 0 6px var(--primary-neon))
        drop-shadow(0 0 10px var(--primary-neon));
    }
    .hitbox {
      pointer-events: all;
      position: absolute;
...
class PromptController extends LitElement {
  static override styles = css`
    ...
    #text {
      ...
      scrollbar-color: var(--tertiary-neon) #0005;
    }
    ...
    :host([filtered]) .prompt {
      --color: #ff3b3b;
      box-shadow: 0 0 5px #ff3b3b, 0 0 10px #ff3b3b, 0 0 20px #ff3b3b,
        0 0 30px #ff3b3b;
    }
  `;
  ...
  private dispatchClearFilter() {
    this.dispatchEvent(
      new CustomEvent<string>('prompt-filter-cleared', {
        detail: this.promptId,
        bubbles: true,
        composed: true,
      }),
  ...
  override render() {
    ...
    return html`<div class="prompt" style=${hostStyles}>
      <button
        class="remove-button"
      ...
      ${
        this.filtered
          ? html`<button
              ...
              @click=${this.dispatchClearFilter}
              title="Clear filter and retry prompt">
              ↻
            </button>`
          : ''
      }
...
class SettingsController extends LitElement {
  static override styles = css`
    :host {
      ...
      border-radius: 5px;
      border: 1px solid var(--ui-border-color);
      box-shadow: 0 0 10px var(--ui-border-color);
      font-family: var(--font-body);
      font-size: 1.5vmin;
      overflow-y: auto;
      scrollbar-width: thin;
      scrollbar-color: var(--tertiary-neon) #1a1a1a;
    ...
    input[type='range']::-webkit-slider-thumb {
      ...
      box-shadow: 0 0 5px var(--thumb-bg), 0 0 8px var(--thumb-bg);
      cursor: pointer;
  ...
  // FIX: Explicitly type `defaultConfig` as `LiveMusicGenerationConfig` to prevent
  // TypeScript from widening `musicGenerationMode` to a generic `string` type,
  // which resolves the type incompatibility.
  private readonly defaultConfig: LiveMusicGenerationConfig = {
    temperature: 1.1,
    topK: 40,
    guidance: 4.0,
    // FIX: Cast 'QUALITY' to the expected type to resolve the type error.
    musicGenerationMode: 'QUALITY' as LiveMusicGenerationConfig['musicGenerationMode'],
  };
...
class PromptDj extends LitElement {
  static override styles = css`
    ...
    .header h1 {
      ...
      text-shadow: 0 0 5px var(--primary-neon), 0 0 10px var(--primary-neon),
        0 0 20px var(--primary-neon), 0 0 40px var(--secondary-neon);
    }
    .prompts-area {
      display: flex;
  ...
  // FIX: Use `(window as any)` to access the vendor-prefixed `webkitAudioContext`
  // for older browser compatibility without causing a TypeScript compilation error.
    ...
    this.session = await ai.live.music.connect({
      ...
      callbacks: {
        onmessage: async (e: LiveMusicServerMessage) => {
          ...
          if (e.serverContent?.audioChunks?.length) {
  ...
  private handlePromptFilterCleared(e: CustomEvent<string>) {
    e.stopPropagation();
    const promptIdToClear = e.detail;
    const prompt = this.prompts.get(promptIdToClear);
    ...
    return html`<div id="background" style=${bg}></div>
      ...
      <div class="prompts-area">
        <div
          ...
          @prompt-filter-cleared=${this.handlePromptFilterCleared}
...
function gen(parent: HTMLElement): PromptDj {
